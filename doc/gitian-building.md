...
*Setup instructions for a gitian build of Saiko using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Saiko
...
Clone the git repositories for saiko and gitian and then checkout the saiko version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/saiko-crypto/saiko.git
cd saiko
...
Follow the instructions in [doc/release-process.md](release-process.md) in the saiko repository
...
To build Saiko (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the saiko repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/saiko/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/saiko-crypto/saiko
...
For example:
```bash
URL=https://github.com/crowning-/saiko.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit saiko=${COMMIT} --url saiko=${URL} ../saiko/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit saiko=${COMMIT} --url saiko=${URL} ../saiko/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit saiko=${COMMIT} --url saiko=${URL} ../saiko/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/saiko-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/saiko-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/saiko-build.assert
```
...
[saiko/gitian.sigs](https://github.com/saiko-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
