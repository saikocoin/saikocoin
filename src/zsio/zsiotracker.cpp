...
CzSIOTracker::CzSIOTracker(std::string strWalletFile)
{
...
CzSIOTracker::~CzSIOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSIOTracker::Init()
{
...
bool CzSIOTracker::Archive(CMintMeta& meta)
{
...
bool CzSIOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSIOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSIOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSIOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSIOTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSIOTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSIOTracker::GetSerialHashes()
{
...
CAmount CzSIOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSIOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSIOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSIOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSIOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSIOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSIOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSIOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSIOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSIOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSIOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSIOWallet* zSIOWallet)
{
    bool iszSIOWalletInitialized = (NULL != zSIOWallet);
    CMintMeta meta;
    ...
    if (! iszSIOWalletInitialized)
        zSIOWallet = new CzSIOWallet(strWalletFile);
    meta.isSeedCorrect = zSIOWallet->CheckSeed(dMint);
    if (! iszSIOWalletInitialized)
        delete zSIOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSIOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSIOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSIOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSIOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSIOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSIOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSIOWallet* zSIOWallet = new CzSIOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSIOWallet);
        }
        delete zSIOWallet;
        LogPrint("zero", "%s: added %d dzsio from DB\n", __func__, listDeterministicDB.size());
    }
