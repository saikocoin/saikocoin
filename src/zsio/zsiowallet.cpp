...
CzSIOWallet::CzSIOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsio seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSIOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSIOSeed_deprecated()) {
                    LogPrintf("%s: Updated zSIO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsio seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsio wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSIOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSIOCount(nCountLastUsed);
    else if (!walletdb.ReadZSIOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSIOWallet::Lock()
{
    seedMaster = 0;
}

void CzSIOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSIOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSIO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSIOWallet::LoadMintPoolFromDB()
{
...
void CzSIOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSIOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSIOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsioTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSIOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSIO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsioTracker which also adds to database
    pwalletMain->zsioTracker->Add(dMint, true);
...
void CzSIOWallet::SeedToZSIO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSIOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSIOWallet::GenerateDeterministicZSIO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSIOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSIOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSIOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
