...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZSIOModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zsioTracker->HasMintTx(hash))) {
            //zSIO stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZSIO;
...
bool IsZSIOType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZSIO:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZSIO || type == TransactionRecord::MNReward) {
