...
#include "ui_zsiocontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zsioTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsioTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSIO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSIO seed used to mint this zSIO is not the same as currently hold in the wallet";
            else
