...
// The zSIO block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSIO is a hash of the serial
...
{
    //Create an output returning the zSIO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSIOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSIO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSIO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSIOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSIO output", __func__);
...
{
    CzSIOTracker* zsioTracker = pwallet->zsioTracker.get();
    CMintMeta meta;
    if (!zsioTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zsioTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SIO stake is the outpoint
