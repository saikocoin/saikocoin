...
public:
    ...
    bool WriteZSIOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSIOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSIOSeed_deprecated(uint256& seed);
    bool EraseZSIOSeed();
    bool EraseZSIOSeed_deprecated();

    bool WriteZSIOCount(const uint32_t& nCount);
    bool ReadZSIOCount(uint32_t& nCount);
