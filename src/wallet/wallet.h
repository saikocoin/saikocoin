...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zsiospends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SIO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSIO send
enum ZerocoinSpendStatus {
    ZSIO_SPEND_OKAY = 0,                            // No error
    ZSIO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSIO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSIO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSIO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSIO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSIO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSIO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSIO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSIO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSIO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSIO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSIO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSIO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSIO_SPENT_USED_ZSIO = 14,                      // Coin has already been spend
    ZSIO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSIO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSIOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzsioAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzSIOTracker> zsioTracker;
    ...
    void setZWallet(CzSIOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zsioTracker = std::unique_ptr<CzSIOTracker>(new CzSIOTracker(strWalletFile));
    }

    CzSIOWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSIO reset */
    boost::signals2::signal<void()> NotifyzSIOReset;
